...
export default function HomePage() {
  ...
  async function copyPrompt(prompt) {
    ...
    toast({
      title: (
        <>
          <Check className="inline-block mr-2 h-5 w-5 text-green-500" /> Prompt
          copied to clipboard
        </>
      ),
    });
  ...
  return (
    <main className="container flex flex-col items-center gap-6 md:gap-20 p-4 md:p-8">
      <header className="md:mt-16 mt-4 space-y-2">
        <h1 className="mt-4 font-heading text-4xl font-bold text-center [text-shadow:_0_4px_0_#e1e1e1] dark:bg-gradient-to-br dark:from-foreground dark:to-gray-500 dark:bg-clip-text dark:text-transparent dark:[text-shadow:none] md:text-5xl lg:text-6xl xl:text-7xl">
          The Community Showcase
        </h1>

        <p className="font-medium md:max-w-lg text-center mx-auto text-muted-foreground md:text-lg">
      ...
      <div className="flex flex-1 justify-end gap-4 md:hidden">
        <Link to="/create" className={cn(buttonVariants(), "shadow")}>
          Create Image
        </Link>
      ...
      <section className="grid xl:grid-cols-4 lg:grid-cols-3 sm:grid-cols-2 grid-cols-1 gap-4">
        {filterPosts(data?.data ?? []).map(({ _id, name, photo, prompt }) => (
          <figure key={_id} className="space-y-4">
            <div className="rounded-lg shadow border hover:shadow-lg p-0.5 transition duration-300 group overflow-hidden hover:-translate-y-1">
              <img
                src={photo}
                className="object-cover aspect-video w-full rounded-md group-hover:scale-105 duration-300 delay-300 ease-in"
              />
            </div>
